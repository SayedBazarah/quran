...
import { useGetCourseById } from 'src/actions/course';
...
export function CourseDetailsView({ id }: Props) {
  const { course, courseLoading } = useGetCourseById(id);
  ...
  if (courseLoading || !course) return <LoadingScreen />;
  return (
    <DashboardContent>
      <CustomBreadcrumbs
        heading="ملف المدرس"
      ...
      <Card sx={{ p: 3 }}>
        ...
        {selectedTab === '' && <CourseDetails course={course} />}
        {/* {selectedTab === 'rounds' && <TeacherRounds rounds={c.courseRounds} />} */}
      </Card>
    </DashboardContent>
  );
}
